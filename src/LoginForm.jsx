...
export default function LoginForm() {
  ...
  const handleLogin = async () => {
    ...
    // 2️⃣ Fetch profile including stock rooms and role
    ...
    setStockRooms(profile.stock_room || []);
    setRole(profile.role || "user");

    alert(`Login successful! Role: ${profile.role}, Assigned stock rooms: ${profile.stock_room.join(", ")}`);
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        width:"400px", backgroundColor:"#fff", padding:"30px", borderRadius:"10px", boxShadow:"0 4px 12px rgba(0,0,0,0.15)"
      }}>
        ...
        {stockRooms.length > 0 && (
          <div style={{textAlign:"center", color:"#333"}}>
            ...
            <p>Assigned Stock Rooms: <strong>{stockRooms.join(", ")}</strong></p>
          </div>
        )}
      </div>
    </div>
